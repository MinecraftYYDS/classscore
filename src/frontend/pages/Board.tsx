...
import { Trophy, RefreshCw, Settings2, Monitor } from "lucide-react";
...
export default function Board() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-indigo-950/60 px-4 py-6 sm:px-8">
            ...
            {[1, 0, 2].map((rank) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`cs-shine relative flex w-full flex-col items-center justify-center overflow-hidden rounded-t-2xl border bg-gradient-to-b transition hover:brightness-110 ${heights[rank]} ${colors[rank]}`}
                  >
          ...
          <AnimatePresence initial={false}>
            {sorted.map((s, i) => (
              <motion.div
                ...
                onClick={() => setDetailId(s.id)}
                className="cs-card flex cursor-pointer items-center gap-3 px-4 py-3 transition hover:border-indigo-500/50"
              >
                <span
      ...
      <StudentDetailModal studentId={detailId} minScore={data?.lottery.minScore ?? 3} onClose={() => setDetailId(null)} />
    </div>
  );
}
